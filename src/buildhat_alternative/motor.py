...
class FrequencyCallback:
    def __init__(self,callback,every,offset):
        self.callback = callback
        self.caller = None
        self.every = every
        self.offset = offset
        
    def doit(self,count,data):
        if count%self.every==self.offset:
            print(count)
            self.callback(data)
...
class Motor:
    ...
    def __init__(self, port, ser, direction=1):
        ...
        self.count = 0
        self.direction = direction
        self.wheel_diameter = 276  # mm  #TODO check wheel diameter
        # self.PIDcontroller = PIDController(0.001,0,0.02)
        self.PIDcontroller = PIDController(0.00076, 0.003, 0.0153)
        self.anglePIDcontoller = PIDController(0.0001,0,0)
        self.speed = 0
    ...
    def handle_data_later(self, speed, pos, apos):
        ...
        # yes you read that right i said "10"
        # to degrees per second.
        self.data = self.format_data(speed,pos,apos)
        self.count +=1
        [listener.doit(self.count,self) for listener in self.listeners]
        speed = self.direction * speed * 10
        self.update(speed,0)
    ...
    def add_listener(self, listener,every,offset):
        """not fully implemented here as an idea only"""
        self.listeners.append(FrequencyCallback(listener,every,offset))

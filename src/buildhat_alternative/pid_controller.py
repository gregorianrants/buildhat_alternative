...
class PIDController:
   def __init__(self, 
      ...
      # self.process_variable = process_variable #going to remove this
      self.manipulated_variable = manipulated_variable
      self.set_point = 0
   ...
   def handle_derivative(self,error):
      derivative = self.derivative_constant*(error - self.previous_error)
   ...
   def update(self,proccess_variable):
      if(self.set_point==0):
         #consider putting in a reset method to do this
         self.manipulated_variable = 0
         self.integral_sum = 0
         self.previous_error = 0
         return self.manipulated_variable
      error = proccess_variable-self.set_point
      self.error = error
     
      adjustment = self.get_value(error)
      self.manipulated_variable = self.manipulated_variable-adjustment
      self.previous_proccess_variable = proccess_variable
      return self.manipulated_variable
